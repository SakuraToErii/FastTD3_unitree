...
Target smoothing:   no clamp (noise is unbounded Gaussian, clamped by noise_clip)
...
When `use_tanh=True`, the critic and replay buffer use the same scaled action space that the environment actually executes. The target actor output is naturally bounded by Tanh, and before critic evaluation it is mapped through `clamp(-1, 1) × action_bounds` (`to_critic_action_space`), so target smoothing and critic inputs stay consistent with the env-facing action contract.

## Files

| File | Role |
|---|---|
| `fast_td3/hyperparams.py` | `use_tanh: bool = False`; `action_bounds` default `None` |
...
| `fast_td3/train.py` | Validation; `to_critic_action_space` for critic/replay consistency; passes `use_tanh` to Actor |
