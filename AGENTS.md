...
python scripts/train_unitree_fasttd3.py \
  ...
  --seed 1
```
`scripts/train.sh` 是批量入口，默认 `SEEDS=3407`、`num_envs=512`、`buffer_size=4096`、`policy_noise=0.1`、`std_max=0.3`，可通过环境变量 `SEEDS` / `UNITREE_RL_LAB_PATH` 覆盖。
...
- **FastTD3**：TD3 + 分布式 Critic（C51 风格，默认 `num_atoms=251`、`v_min=-10`、`v_max=10`）+ Clipped Double Q（`use_cdq=True`）。Actor 输出确定性动作并加 clipped Gaussian 探索噪声（`policy_noise`/`noise_clip`/`std_min`/`std_max`）。延迟策略更新 `policy_frequency=2`，每步 `num_updates=4` 次梯度。Actor 输出激活由 `use_tanh` 控制（默认 `False`，无界输出对齐 PPO；`True` 时附加 Tanh 并配合 `action_bounds` 缩放）。
...
| `gamma / tau` | 0.99 / 0.1 | |
| `policy_noise / noise_clip / std_max` | 0.2 / 0.5 / 0.3 | |
